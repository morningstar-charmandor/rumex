...
- **⭐ Current approach (2026-07, VALIDATED): honest top-level login window.** Google sign-in is done in a dedicated top-level window (`openHonestLoginWindow`, `src/main/loginWindow.ts`), wired from `main` via `isGoogleSignInUrl`: when a Google webview heads to a sign-in URL its navigation is cancelled and the login runs in a real `BrowserWindow` webContents, **sharing the webview's session**, then the panel is returned to its app URL, authenticated. Tested on a real machine across personal **and Google Workspace** accounts — Workspace never worked through the Firefox disguise; honest does. Two non-obvious requirements (see `login-test/FINDINGS.md`): **(a)** the UA must strip **only** the Electron token and **keep the app token** (`LOGIN_HONEST_UA`) — the fully-cleaned pure-Chrome `CHROME_UA` is what Google rejects; **(b)** it must be a **top-level `BrowserWindow`**, not a `WebContentsView`/`<webview>`. Also required for usability: restore the panel's real URL after login (not `about:blank`), and a 12s cooldown after the window closes to avoid a re-challenge flicker loop. Firefox disguise is **off** for Google webviews now.
- **The Firefox disguise (below) is retained only for third-party OAuth popups** (`setWindowOpenHandler`) as a fallback; it is no longer the Google web-app login path.
- **Historical note:** a cleaned Chrome UA is rejected — Google cross-checks claimed-Chrome against Chrome-only signals; the mismatch is the tell. This is why the honest window keeps the app token (so the UA does not read as a bare Chrome claim) and why the embedded webview couldn't just drop the disguise.
