...
- **⭐ Update (2026-07): honest top-level login window.** Empirical A/B testing (see `login-test/` + `login-test/FINDINGS.md`) found that a *real top-level window* presenting the **honest** Chrome UA (no disguise) signs in reliably — including Google **Workspace / custom-domain** accounts, where the Firefox disguise **fails outright** ("couldn't sign you in"). So a Google app's webview no longer attempts login itself: when it heads to a Google **sign-in** URL (`isGoogleSignInUrl`), `main` opens `openHonestLoginWindow` (`src/main/loginWindow.ts`) — a top-level `WebContentsView` on the Google page, honest Chrome UA, **sharing the webview's own session** so the cookie lands in the right partition, with a read-only address+lock bar and no page injection. Completion is detected from the session's auth cookie or a forward off `accounts.google.com`; then the window closes and the app webview reloads, authenticated. The Firefox UA below is **kept as-is** for the running app webview and for third-party OAuth popups (untested surfaces — do not remove it, and do not simply drop the disguise from the embedded webview: that surface was the original failing case).
- **Present as Firefox on Google surfaces.** A cleaned Chrome UA is NOT enough *in the embedded webview* — Google cross-checks claimed-Chrome against Chrome-only signals; the mismatch is itself the tell. (This is why login is handed to the honest top-level window above rather than done in-webview.)
